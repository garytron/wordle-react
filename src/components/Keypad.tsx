  ...
  return (
    <>
      <div className='keypad flex flex-wrap max-w-full md:max-w-[580px] m-[20px] mx-auto bg-[#DADCE04D] dark:bg-[#DADCE008] rounded-md py-4'>
        <div className='w-full flex justify-center md:justify-start md:ml-9'>
        ...
        <div className='w-full flex justify-center md:justify-start md:ml-5'>
          {letters.slice(20).map((l) => {
              ...
              if(l.special) {
                if(l.key === 'Backspace')
                  return <div key={l.key} className={`key md:min-w-[80px] h-[50px] flex m-[5px] dark:hover:bg-[rgba(255,255,255,0.2)] dark:bg-[#565F7E] dark:text-white hover:bg-gray-300 px-[10px] md:px-[15px] font-medium text-center rounded-[6px] leading-[50px] items-center justify-center cursor-pointer ${color} ${color? 'text-white' : ''}`} onClick={() => onClickLetter('Backspace')}><Icon>backspace</Icon></div>
                
                return <div key={l.key} className={`key md:min-w-[80px] h-[50px] inline-block m-[5px] dark:hover:bg-[rgba(255,255,255,0.2)] dark:bg-[#565F7E] dark:text-white hover:bg-gray-300 px-[10px] md:px-[15px] font-medium text-center rounded-[6px] leading-[50px] cursor-pointer ${color} ${color? 'text-white' : ''}`} onClick={() => onClickLetter(l.key)}>{l.key}</div>
              }
